...
$sql = "SELECT nome, descricao, imagem FROM servicos"; 
        ...
        <div class="cards-list"> 
            ...
            if ($result->num_rows > 0) { 
                ...
                while ($row = $result->fetch_assoc()) { 
                    ...
                    // Verificar se a imagem existe no diretório antes de exibir
                    $imagem_path = 'img/' . $imagens;
                    if (file_exists($imagem_path) && !empty($imagens)) {
                        echo '<div class="card-item">'; // Card
                        echo '<img src="' . $imagem_path . '" alt="' . $nome_servico . '">'; // Imagem do serviço
                        echo '<h3>' . $nome_servico . '</h3>'; // Nome do serviço
                        echo '<p>' . $descricao_servico . '</p>'; // Descrição do serviço
                        echo '</div>'; // Fechar card
                    } else {
                        echo '<div class="card-item">'; // Card sem imagem
                        echo '<img src="img/default.jpg" alt="Imagem não disponível">'; // Imagem padrão
                        echo '<h3>' . $nome_servico . '</h3>'; // Nome do serviço
                        echo '<p>' . $descricao_servico . '</p>'; // Descrição do serviço
                        echo '</div>'; // Fechar card
                    }
